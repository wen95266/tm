...
if [ -f ".env" ]; then
    ...
    read -p "请输入 Alist Token (可选): " alist_token
    read -p "请输入 RTMP 推流地址 (可选): " rtmp_url
    ...
    if [ ! -z "$alist_token" ]; then
        echo "ALIST_TOKEN=$alist_token" >> .env
    fi
    if [ ! -z "$rtmp_url" ]; then
        echo "RTMP_URL=$rtmp_url" >> .env
    fi
    echo -e "\033[1;32m配置已保存！\033[0m"
fi

# 5. Start the CLI application in auto-install mode
